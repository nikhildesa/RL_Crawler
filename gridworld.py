...
###############################################################################
    ...
    def Check_changes(old_v,v):
        change = np.abs(np.array(old_v) - np.array(v))
        if np.sum(change) < 1e-4:  # check line 80
            return False

    for i in range(max_iterations):
        old_v = v[:]   # shallow copy, orelse was v changes old_v will change.
        for state in range(NUM_STATES):
            all_actions = [0] * NUM_ACTIONS
            
            for a in range(NUM_ACTIONS):
                for probabilty,newstate,reward,terminal in TRANSITION_MODEL[state][a]:
                    all_actions[a]+= (probabilty * (reward + gamma * old_v[newstate]))

            optimal_action = max(all_actions)
            v[state] = optimal_action


        check = Check_changes(old_v,v)    # check if it is converged or not
        if check == False:
            break
        ...
        for state in range(NUM_STATES):
            ...
            for a in range(NUM_ACTIONS):
                for probabilty,newstate,reward,terminal in TRANSITION_MODEL[state][a]:
                    all_actions[a]+= (probabilty * (reward + gamma * v[newstate]))
            
            max_value = max(all_actions)
            max_index = all_actions.index(max_value)
            pi[state] = max_index               # give the action to the policy
        logger.log(i,v,pi)
...
###############################################################################
    ...
    for i in range(0,NUM_STATES):
        pi.append(random.randint(0, NUM_ACTIONS - 1))
    ...
    for i in range(max_iterations):
        while True:
            ...
            for state in range(NUM_STATES):
                ...
                for probability,newstate,reward,terminal in TRANSITION_MODEL[state][pi[state]]:
                    action_value+= probability * (reward + gamma * old_v[newstate])
                
                v[state] = action_value
            
            check = Check_chang(old_v,v)
            if check == False:
                print('check')
                break
        ...
        for state in range(NUM_STATES):
           ...
           if max_index!=current_action:
              policy_stable = False
           pi[state] = max_index
        
        logger.log(i,v,pi)
        if policy_stable == True:   # no more changes are going to happen
